...
def build_dataset(df: pd.DataFrame, feature_dim: int, num_patches: int = 10):
...
def build_tft(dataset: TimeSeriesDataSet, hidden_size: int = 64, dropout: float = 0.1):
...
def train_tft(tft, train_dataset: TimeSeriesDataSet, val_dataset: TimeSeriesDataSet,
...
def extract_latent_and_attention(tft, dataset: TimeSeriesDataSet, batch_size: int = 64):
...
def project_umap(latents: torch.Tensor, n_components: int = 2, random_state: int = 42):
    ...
    coords  = reducer.fit_transform(latents.numpy())
    return coords


def run_stage3(patched_data: dict, feature_dim: int, num_patches: int = 10,
               hidden_size: int = 64, max_epochs: int = 30, batch_size: int = 64):

    df = build_dataframe(patched_data)

    window_ids = df["window_id"].unique()
    n_train    = int(len(window_ids) * 0.8)
    train_ids  = set(window_ids[:n_train])

    train_df = df[df["window_id"].isin(train_ids)].reset_index(drop=True)
    val_df   = df[~df["window_id"].isin(train_ids)].reset_index(drop=True)
    ...
    tft = train_tft(tft, train_dataset, val_dataset, max_epochs=max_epochs,
